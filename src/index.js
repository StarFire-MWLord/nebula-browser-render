...
app.get("/api/settings",(_req,res)=>res.set("Cache-Control","no-store").json(sharedSettings));
app.put("/api/settings",(req,res)=>{
  ...
  sharedSettings={
    ...
    mouseSensitivity:Math.max(.1,Math.min(10,Number(b.mouseSensitivity)||1.5)),
    animationSpeed:Math.max(.1,Math.min(10,Number(b.animationSpeed)||1.5)),
    freezeMode:String(b.freezeMode||sharedSettings.freezeMode)
  };
  res.json(sharedSettings);
});
app.use((req,res,next)=>{if(["/","/index.html","/app.js","/sw.js","/uv.config.js"].includes(req.path))res.setHeader("Cache-Control","no-store, max-age=0");next();});
...
const server=createServer((req,res)=>{res.setHeader("Cross-Origin-Opener-Policy","same-origin");res.setHeader("Cross-Origin-Embedder-Policy","require-corp");app(req,res);});
server.on("upgrade",(req,socket,head)=>{const pathname=new URL(req.url||"/","http://localhost").pathname;if(pathname==="/wisp/"){req.url=pathname;wisp.routeRequest(req,socket,head);return}socket.end();});
