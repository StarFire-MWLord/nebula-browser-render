...
async function setupProxy() {
  ...
  // IMPORTANT: root scope matches the official Ultraviolet app. The previous
  // /service/ scope + navigator.serviceWorker.ready could wait forever on "/".
  const reg = await navigator.serviceWorker.register("/sw.js");
  await waitForActivation(reg);

  const conn = new BareMuxConnection("/baremux/worker.js");
  const wisp = `${location.protocol === "https:" ? "wss" : "ws"}://${location.host}/wisp/`;
  if ((await conn.getTransport()) !== "/epoxy/index.mjs") {
    await conn.setTransport("/epoxy/index.mjs", [{ wisp }]);
  }
}

let readyPromise = setupProxy();
readyPromise.then(() => status.textContent = "Ready")
  .catch(err => { console.error(err); status.textContent = `Proxy setup failed: ${err.message || err}`; });
