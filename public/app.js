...
function waitForActivation(r){if(r.active)return Promise.resolve();const w=r.installing||r.waiting;if(!w)return Promise.resolve();return new Promise((ok,no)=>{const t=setTimeout(()=>no(new Error("Service worker activation timed out")),12000);w.addEventListener("statechange",()=>{if(w.state==="activated"){clearTimeout(t);ok()}if(w.state==="redundant"){clearTimeout(t);no(new Error("Service worker became redundant"))}})})}
